...
class Predictor:
    def __init__(self):
        ...
        self.inconsistent_analysis = None  # 新增：存储不一致分析结果
    ...
    def analyze_inconsistent_features(self, X_original, y_true, y_pred, output_column):
        """
        分析预测不一致的特征原因

        Args:
            X_original: 原始特征数据（预处理前）
            y_true: 真实标签
            y_pred: 预测结果
        ...
        try:
            # 找到预测不一致的样本索引
            inconsistent_mask = y_true != y_pred
            ...
            print(f"发现 {len(inconsistent_indices)} 个预测不一致的样本")
            ...
            for idx in inconsistent_indices:
                sample_features = X_original.iloc[idx]
                ...
                for feature in X_original.columns:
                    if feature in feature_importance:
                        # 简单异常度计算：特征值 * 特征重要性
                        # 实际应用中可以使用更复杂的SHAP值等方法
                        feature_value = sample_features[feature]
                        importance = feature_importance[feature]

                        # 对数值特征，计算与均值的偏差
                        if feature in self.numeric_features:
                            mean_val = X_original[feature].mean()
                            deviation = abs(feature_value - mean_val) / (X_original[feature].std() + 1e-8)
                            score = deviation * importance
                        else:
                            # 对分类特征，使用频率统计
                            freq = (X_original[feature] == feature_value).mean()
                            score = (1 - freq) * importance

                        feature_scores[feature] = score

                # 找到贡献最大的特征
                if feature_scores:
                    main_feature = max(feature_scores.items(), key=lambda x: x[1])[0]
                    main_feature_value = sample_features[main_feature]

                    # 记录特征贡献频次
                    if main_feature not in feature_contributions:
                        feature_contributions[main_feature] = 0
                    feature_contributions[main_feature] += 1

                    inconsistent_details.append({
                        'index': idx,
                        'serial': self.results_df.iloc[idx][
                            'Serial'] if 'Serial' in self.results_df.columns else f"样本_{idx}",
                        'true_label': "Normal" if true_label == 1 else "PA abnormal",
                        'pred_label': "Normal" if pred_label == 1 else "PA abnormal",
                        'main_feature': main_feature,
                        'feature_value': main_feature_value,
            ...
            return {
                'total_inconsistent': len(inconsistent_indices),
                'feature_frequency': dict(sorted_features),
                'sample_details': inconsistent_details,
                'output_column': output_column
            }

        except Exception as e:
            print(f"分析不一致特征时出错: {str(e)}")
            return {
                'total_inconsistent': 0,
                'feature_frequency': {},
                'sample_details': [],
    ...
    def _get_feature_importance(self):
        ...
        try:
            if self.model_type == "xgboost" and hasattr(self.model, 'get_score'):
                importance_dict = self.model.get_score(importance_type='weight')
                total = sum(importance_dict.values())
                if total > 0:
                    feature_importance = {k: v / total for k, v in importance_dict.items()}

            elif self.model_type == "lightgbm" and hasattr(self.model, 'feature_importance'):
                importance_array = self.model.feature_importance(importance_type='split')
                if hasattr(self.model, 'feature_name'):
                    feature_names = self.model.feature_name()
                    total = sum(importance_array)
                    if total > 0:
                        feature_importance = {name: imp / total for name, imp in zip(feature_names, importance_array)}

            elif self.model_type == "catboost" and hasattr(self.model, 'get_feature_importance'):
                importance_array = self.model.get_feature_importance()
            ...
            elif self.model_type == "sklearn" and hasattr(self.model, 'feature_importances_'):
        ...
        # 如果无法获取特征重要性，使用均匀分布
        if not feature_importance and hasattr(self, 'model_features') and self.model_features:
            uniform_importance = 1.0 / len(self.model_features)
            feature_importance = {feature: uniform_importance for feature in self.model_features}
    ...
    def run_prediction(self):
        ...
        try:
            ...
            self.results_df['Prediction Result'] = predictions
            self.results_df['Prediction Label'] = self.convert_prediction_to_label(predictions)

            # 执行不一致特征分析
            self._perform_inconsistency_analysis(X, predictions)
    ...
    def _perform_inconsistency_analysis(self, X_original, predictions):
        """执行预测不一致的特征分析"""
        self.inconsistent_analysis = {}

        # 检查所有输出特征列
        output_columns = ['PA Status Pattern 1', 'PA Status Pattern 2', 'PA Status Repair Info']

        for output_col in output_columns:
            if output_col in self.results_df.columns:
                # 确保标签是数值类型
                y_true = self.results_df[output_col].copy()
                ...
                if y_true.dtype == 'object':
                    def convert_label(label):
                        ...
                        if 'Normal' in label_str:
                            return 1
                        elif 'Non PA Issue' in label_str:
                            return 1
                        else:
                            return 0
                ...
                # 分析不一致特征
                analysis_result = self.analyze_inconsistent_features(
                    X_original, y_true, predictions, output_col
                )

                self.inconsistent_analysis[output_col] = analysis_result
    ...
    def get_product_accuracy_report(self):
        """获取产品层面的准确性报告"""
        if self.results_df is None:
            return "没有可用的预测结果"

        # 检查必要的列是否存在
        required_columns = ['Serial', 'Prediction Result']
        output_columns = ['PA Status Pattern 1', 'PA Status Pattern 2', 'PA Status Repair Info', 'Repair Center Info']

        # 找到存在的输出特征列
        available_output_columns = [col for col in output_columns if col in self.results_df.columns]

        if 'Serial' not in self.results_df.columns:
            return "错误: 数据中缺少Serial列，无法进行产品层面统计"

        if not available_output_columns:
            return "错误: 数据中缺少输出特征列，无法进行准确性统计"

        report = "\n2. 产品(product)层面准确性:\n"
        report += "-" * 30 + "\n"

        # 为每个输出特征分别计算产品层面准确性
        for output_col in available_output_columns:
            report += f"\n输出特征: {output_col}\n"
            report += "-" * 30 + "\n"

            # 获取产品层面统计结果
            product_stats = self._compute_product_accuracy(output_col)

            if product_stats is None:
                report += "  无法计算产品层面准确性\n"
                continue

            accuracy = product_stats['accuracy']
            total_products = product_stats['total_products']
            consistent_products = product_stats['consistent_products']
            inconsistent_serials = product_stats['inconsistent_serials']

            report += f"  产品总数: {total_products}\n"
            report += f"  一致产品数: {consistent_products}\n"
            report += f"  产品层面准确率: {accuracy:.4f}\n"

            if inconsistent_serials:
                report += f"\n  预测不一致的产品串号(Serial)列表 ({len(inconsistent_serials)}个):\n"
                for i, serial in enumerate(inconsistent_serials, 1):
                    report += f"    {i}. {serial}\n"
            else:
                report += "\n  所有产品预测一致\n"
    ...
    def _compute_product_accuracy(self, output_column):
        """计算产品层面准确性
        Args:
            output_column: 输出特征列名
        Returns:
        ...
        try:
            ...
            df['pred_label'] = df['Prediction Result']

            # 按Serial分组
            product_groups = df.groupby('Serial')

            product_results = []
            inconsistent_serials = []

            for serial, group in product_groups:
                if len(group) == 0:
                    continue

                # 统计真实标签的多数类别(多数：>50%)
                true_counts = group['true_label'].value_counts()
                true_majority = true_counts.idxmax() if true_counts.max() / len(group) > 0.5 else None

                # 统计预测标签的多数类别
                pred_counts = group['pred_label'].value_counts()
                pred_majority = pred_counts.idxmax() if pred_counts.max() / len(group) > 0.5 else None

                # 只有当两个都有明确的多数类别时才进行比较
                if true_majority is not None and pred_majority is not None:
                    is_consistent = (true_majority == pred_majority)
                    ...
                    if not is_consistent:
                        inconsistent_serials.append(serial)

            if not product_results:
                return None

            # 计算准确率
            consistent_count = sum(1 for result in product_results if result['consistent'])
            ...
            return {
                ...
                'consistent_products': consistent_count,
                'inconsistent_serials': inconsistent_serials,
                'detailed_results': product_results
            }

        except Exception as e:
            print(f"计算产品层面准确性时出错: {str(e)}")
            return None

    def get_inconsistency_analysis_report(self):
        """获取不一致性分析报告"""
        if not self.inconsistent_analysis:
            return "没有可用的不一致性分析结果"

        report = "\n3. 预测不一致性特征分析:\n"
        report += "=" * 50 + "\n"

        for output_col, analysis in self.inconsistent_analysis.items():
            if analysis['total_inconsistent'] == 0:
                report += f"\n输出特征: {output_col}\n"
                report += "-" * 30 + "\n"
                report += "  所有条目预测一致，无需分析\n"
                continue

            report += f"\n输出特征: {output_col}\n"
            report += "-" * 30 + "\n"
            report += f"  预测不一致条目数: {analysis['total_inconsistent']}\n\n"

            # 显示主要特征频次统计
            if analysis['feature_frequency']:
                report += "  导致不一致的主要特征频次统计:\n"
                for feature, count in list(analysis['feature_frequency'].items())[:10]:  # 显示前10个
                    percentage = (count / analysis['total_inconsistent']) * 100
                    report += f"    {feature}: {count} 次 ({percentage:.1f}%)\n"

                # 显示不一致条目详情
                report += f"\n  不一致条目详情 (共{len(analysis['sample_details'])}个):\n"
                for i, detail in enumerate(analysis['sample_details']):
                    report += f"    [{i + 1}] 条目索引: {detail['index']}, Serial: {detail['serial']}\n"
                    report += f"        真实标签: {detail['true_label']}, 预测标签: {detail['pred_label']}\n"
                    report += f"        主要特征: {detail['main_feature']} = {detail['feature_value']}\n"

                    # 显示前3个最重要的特征贡献度
                    sorted_scores = sorted(detail['feature_scores'].items(), key=lambda x: x[1], reverse=True)[:3]
                    report += f"        特征贡献度TOP3:\n"
    ...
    def get_accuracy_report(self):
        ...
        for col in output_columns:
            if col in self.results_df.columns:
                ...
                accuracy = accuracy_score(
                    y_true,
                    self.results_df['Prediction Result']
                )
                report += f"与 {col} 比较:\n"
        ...
        # 产品(product)层面准确性
        product_report = self.get_product_accuracy_report()
        report += product_report

        # 不一致性特征分析
        inconsistency_report = self.get_inconsistency_analysis_report()
    ...
    def clear_results(self):
        ...
        self.encoder = None
        self.metadata = None
        self.inconsistent_analysis = None
