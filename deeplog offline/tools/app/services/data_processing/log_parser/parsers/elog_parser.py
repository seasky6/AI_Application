...
def _process_value(value: str) -> Tuple[LogValue, str, bool]:
    ...
    unit_patterns = [
        # 匹配以W结尾的功率值，如 "40W"
        (r'^(\d+)W$', 'int', None),
        # 匹配dBm值，可能后面跟着其他单位，如 "47.23 dBm[53W]"
        (r'^([\d.]+)\s*dBm', 'float', None),
        # 匹配需要单位转换的值，如 "579 [0.1C]"
        (r'^(\d+)\s*\[', 'float', 0.1),
        # 新增：匹配带小数点的需要单位转换的值
        (r'^([\d.]+)\s*\[', 'float', 0.1)
    ]

    for pattern, vtype, conversion_factor in unit_patterns:
        ...
        if match:
            try:
                ...
                if conversion_factor is not None:
                    num = round(num * conversion_factor, 1)
